...
**Does:** add a "Connect Google" action that launches a Custom Tab (never a WebView) at the backend's `GET /oauth/google/start`, and a deep-link receiver for `sableapp://oauth-complete` (already reserved in the `AndroidManifest.xml` intent-filter from phase 00) that closes the Custom Tab and shows "Connected." No token handling on this side at all — the backend does the whole exchange and stores tokens itself.
